...
const fs = require('fs');
const buf = fs.readFileSync('public/product/websitefeatures.riv');
...
// Scan for LinearAnimation (0x1f) type bytes and read following properties
const animations = [];
let offset = 0;

while (offset < buf.length - 10) {
    if (buf[offset] === 0x1f) {
        // Try to read properties until we hit a 0x00 terminator
        let p = offset + 1;
        let name = null, fps = null, duration = null, loopType = null;
        let maxRead = 40; // max property bytes to scan from this position
        ...
        while (p < offset + maxRead && p < buf.length) {
            ...
            if (propKey === 0x37) { // name (string)
                ...
                break; // unknown key, stop parsing this object
            }
        }
...
console.log('=== All LinearAnimations in websitefeatures.riv ===\n');
