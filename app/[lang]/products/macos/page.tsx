...
import { MacosScrollUI } from '@/components/shared/MacosScrollUI'
import { MacosWhySection } from '@/components/shared/MacosWhySection'
import { MacosBestBlockerSection } from '@/components/shared/MacosBestBlockerSection'
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    return {
        title: 'BlockP for MacOS - Porn Blocker & Content Filter',
        description: 'Download BlockP for MacOS. AI-powered porn blocking, accountability features, and streak tracking to help you stay focused.',
        alternates: {
            canonical: `/${locale}/products/macos`,
            languages: buildLocaleAlternates((supportedLocale) => `/${supportedLocale}/products/macos`),
        },
    }
}

export default async function MacosProductPage({ params }: { params: Promise<{ lang: string }> }) {
    const { lang } = await params
    const locale = resolveLocale(lang)

    // Fetch data from Sanity, gracefully fallback to hardcoded if null
    const product = await sanityClient.fetch<any>(PRODUCT_BY_SLUG_QUERY, { slug: 'macos', lang: locale })

    const rawHeroTitle = product?.heroTitle || ""
    let heroTitle1 = "BlockP"
    let heroTitle2 = "for MacOS."
    if (rawHeroTitle) {
    ...
    return (
        <div className="w-full flex flex-col items-center bg-[#F6FAFF] min-h-screen overflow-x-clip">
            ...
            >
                ...
                {/* Main Graphic - Shifted slightly higher and LEFT */}
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full flex justify-center z-10 px-6 md:-translate-x-[calc(50%+80px)] lg:-translate-x-[calc(50%+160px)] mb-[-50px] md:mb-[-80px] lg:mb-[-100px]">
                    <div className="w-[580px] md:w-[640px] lg:w-[980px] h-auto">
                        <img
                            src={product?.heroImageUrl || "/product/macos/macos.svg"}
                            alt="BlockP for MacOS"
            ...
            <MacosWhySection data={product?.whySection} />

            {/* ── Why BlockP Is The Best Porn Blocker App Section ── */}
            <MacosBestBlockerSection data={product?.bestBlockerSection} />

            {/* ── Benefits of using a porn blocker Section ── */}
            <MacosBenefitsSection data={product?.benefitsSection} />

            {/* ── FAQs Section ── */}
            <MacosFaqsSection data={product?.faqsSection} />
        </div>
    )
}
